...
export class RecoveryDialog extends FormApplication {
  ...
  static get defaultOptions() {
      ...
      classes: ["numenera"],
      title: "Recovery",
      template: "systems/cypher/templates/dialog/recovery.html",
      closeOnSubmit: false,
      submitOnChange: true,
      submitOnClose: false,
      editable: true,
  ...
  getData() {
    ...
    const recoveriesData = Object.entries(CYPHER.recoveries)
      .map(([key, value], idx) => {
        return {
          key,
          label: value,
    ...
    for (const prop in CYPHER.stats) {
      stats[prop] = game.i18n.localize(CYPHER.stats[prop]);
    }
  ...
  _getFormula(n) {
    ...
    let constant;
    if (game.settings.get("cypher", "cypherShort")) {
      constant = 1;
    } else {
      constant = n * this.object.actor.data.data.tier;
    }
