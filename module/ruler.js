...
function useCypherDistances() {
    let units = game.settings.get("cypher", "measureDistanceInUnits");
    if (units === "none") {
        return false;
    }
...
function getDistanceBandInFeet(distance) {
    if (distance <= 10.0) {
        return game.i18n.localize("CYPHER.range.immediate");
...
function getCypherSegmentLabel(segmentDistance, totalDistance, isTotal) {
    ...
    let distanceBand = getDistanceBand(game.settings.get("cypher", "measureDistanceInUnits"), distance);
    if ( isTotal ) {
...
function cypherRefreshRulerText() {
    ...
    let distanceBand;
    let cypherUnit = game.settings.get("cypher", "measureDistanceInUnits");
    if ( this.data.t === "rect" ) {
